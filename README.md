...
The algorithm seeks to find a non-negative matrix $H_{n \times k}$ that minimizes the squared Frobenius norm:


$$min_{H \ge 0} ||W - HH^T||_F^2$$

Where $W$ is the normalized similarity matrix derived from the input data $X$. The update rule is implemented using a multiplicative approach:
